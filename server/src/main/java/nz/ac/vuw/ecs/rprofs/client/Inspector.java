...
public class Inspector implements EntryPoint, HasHandlers {
	...
	private EventFactory<ArrayList<RunData>> profilerRunCallback =
		new EventFactory<ArrayList<RunData>>(this, "Failed to perform run callback!") {
		@Override
		public ProfilerRunEvent createEvent(ArrayList<RunData> result) {
	...
	public void dropProfilerRun(RunData run) {
		inspector.dropProfilerRun(run, profilerRunCallback);
	}

	public void stopProfilerRun(RunData run) {
	...
	public void getClasses(RunData run) {
	...
	public void getInstanceInformation(RunData run, InstanceData id) {
	...
	private class LogRequestCallback implements LogCallback {
		...
		public LogRequestCallback(RunData run, int type, ClassData cls, LogListener listener) {
			this.run = run;
			this.type = type;
			this.cls = cls;
	...
	public void getLogs(final RunData run, final int type, final ClassData cls, final LogListener listener) {
		if (run == null) return;
	...
	private class ReportCallback<T> implements ReportListener.ReportCallback<T> {
		...
		public ReportCallback(Report report, RunData run, ReportListener<T> listener) {
			this.report = report;
			this.run = run;
			this.listener = listener;
		}
	...
	public <T> void getReportData(RunData run, Report report, Report.Entry key, T target, ReportListener<T> listener) {
		if (run == null) return;
		new ReportCallback<T>(report, run, listener).getAvailable(key, target);
	}

	public <T> void getReportStatus(final RunData run, final Report report, final ReportListener<T> listener) {
		if (run == null) return;
	...
	public <T> void generateReport(final RunData run, final Report report, final ReportListener<T> listener) {
		if (run == null) return;
