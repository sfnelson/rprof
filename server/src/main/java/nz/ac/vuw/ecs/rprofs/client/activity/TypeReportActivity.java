...
public abstract class TypeReportActivity<T extends ReportPlace<T>> extends ReportActivity<T> implements ReportView.Presenter {

	public TypeReportActivity(ProfilerFactory factory, T place) {
		super(factory, place);
	}

	protected void findPackages() {
		DatasetRequest rq = getFactory().getRequestFactory().datasetRequest();
		rq.findPackages(getDataset())
