...
public class InspectorServiceImpl extends RemoteServiceServlet implements InspectorService {
	...
	@Override
	public ArrayList<RunData> getProfilerRuns() {
		ArrayList<RunData> profiles = Collections.newList();
		for (RunInfo run: Context.getRuns()) {
	...
	@Override
	public ArrayList<RunData> dropProfilerRun(RunData run) {
		Context.dropRun(run);
	...
	@Override
	public ArrayList<RunData> stopProfilerRun(RunData run) {
		Context.stop();
	...
	@Override
	public ArrayList<ClassData> getClasses(RunData run) {
	...
	@Override
	public ArrayList<LogData> getLogs(RunData run, int type, ClassData cls, int offset, int limit) {
	...
	@Override
	public int getNumLogs(RunData run, int type, ClassData cls) {
	...
	@Override
	public Status generateReport(Report report, RunData run) {
		return Context.getInstance(run).getReport(report).generate();
	}

	@Override
	public Integer getReportData(Report report, RunData run, Report.Entry key) {
	...
	@Override
	public ArrayList<? extends Report.Entry> getReportData(Report report, RunData run, Report.Entry key, int offset, int limit) {
	...
	@Override
	public Status getReportStatus(Report report, RunData run) {
	...
	@Override
	public ExtendedInstanceData getInstanceInformation(RunData run, InstanceData instance) {
		return Context.getInstance(run).getInstanceInformation(instance);
	}
