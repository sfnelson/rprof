...
public class HistoryManager implements HasValueChangeHandlers<History>, ValueChangeHandler<String>, ProfilerRunHandler {
	...
	private Map<String, RunData> runs = Collections.newMap();
	...
	public void profilerRunsAvailable(List<RunData> result) {
		runs.clear();
		for (RunData run: result) {
			runs.put(run.getHandle(), run);
		}
	...
	private void parseToken(History history, String input, int from, int to) {
		...
		if (name.equals("run")) {
			...
			history.id = new InstanceData(Long.parseLong(value), null);
		}
