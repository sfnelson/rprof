...
static void testFieldTable()
{
    ...
    printf("Field Tests: %llu/%llu assertions passed\n", passed, count);
    passed = 0;
    count = 0;
}
...
static void
class_visitor(jvmtiEnv *jvmti, JNIEnv *jni, const char* cname, jint cid, jint props)
{
    assert(10, (uint64_t) class_count, (uint64_t) (cname[0] - 'a'));
    assert(20, class_count+1, (uint64_t) cid);
    assert(30, class_count+2, (uint64_t) props);
    class_count++;
}
